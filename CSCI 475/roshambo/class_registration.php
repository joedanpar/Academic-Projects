...
//check if member ixists for registration
$member_found = mysql_query("SELECT MEMBER_ID FROM MEMBER WHERE MEMBER_ID = '$member_id';")
  or die("Unable to query member ID");

  while($row = mysql_fetch_assoc($member_found))
    $member_id_number = $row["MEMBER_ID"];

  if($member_id_number != $member_id)
    {
    die("<center>Member ID Not found</center>");
    }
...
$password_retrieve = mysql_query("SELECT PASSWORD FROM PASSWORD WHERE MEMBER_ID = '$member_id'")
  ...
  $remove_seat = mysql_query("SELECT AVAILABILITY FROM SESSION WHERE REF_NUM = '$course';")
    or die("Unable to get availability from database");

  while($row = mysql_fetch_assoc($remove_seat))
    $availability = $row["AVAILABILITY"];

  if($availability == 0)
    die("<br><center>The course is currently full</center>");


mysql_query ("INSERT INTO ENROLLED(MEMBER_ID, REF_NUM) VALUES('$member_id','$course');")
   or die ("<center>Unble to insert into enrolled. Are you already registered for this course?</center>");

$result = mysql_query("SELECT MEMBERSHIP_TYPE FROM MEMBER WHERE MEMBER_ID = '$member_id'")
   or die ("Unable to get membership level");

while ($row = mysql_fetch_assoc($result))
  $mlevel = $row["MEMBERSHIP_TYPE"];
...
$result = mysql_query("SELECT BALANCE FROM MEMBER WHERE MEMBER_ID = '$member_id'")
  or die ("Unable to get balance");
...
mysql_query ("UPDATE MEMBER SET BALANCE = '$balance' WHERE MEMBER_ID = '$member_id';")
  or die ("Unable to insert balance");

echo "<center>Successfully registered for course $course </center>";

?>

<p align="center"><a href="payment_information.html">Make A Payment</a><br>
<a href="class_registration.html">Go Back</a></p>
