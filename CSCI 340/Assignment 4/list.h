...
/*template <class T>
void List<T>::Insert(const Student & s)
	{
	Node<T> *temp = new Node<T>;
	temp->data.set_name(s.get_name());
	temp->data.set_ssn(s.get_ssn());
	temp->data.set_DOB(s.get_DOB());
	temp->data.set_gender(s.get_gender());
	temp->data.set_Zid(s.get_Zid());
	temp->next = headptr;
	headptr = temp;
	}*/
...
************************************************************************************/
template <class T>
Node<T> * List<T>::get_headptr() const
    {
    return headptr;
    }
